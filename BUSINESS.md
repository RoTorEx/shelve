...
Sections follow config order. Pinned folders follow pin order, then remaining
immediate non-hidden child directories follow case-insensitive alphabetical
order. Pinned and discovered paths are deduplicated. Automatic discovery is
deliberately shallow; important deeper folders are explicit pins. Letters
continue after Z as AA, AB, etc.; folder positions are one-based. Selectors are
positions, not permanent IDs. Both open and move use the full materialized
config to assign codes, so PDF filtering never changes a destination's selector
or allows an open-only folder. Every `move_here` path must be a pin or a
discovered child.
